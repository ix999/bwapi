...
namespace BWAPI
{
  ...
  class UnitImpl : public UnitInterface
  {
    ...
    // data members
      ...
      // BOT_MIRROR_SEQ (read once): run the fingerprint build in engine-index order.
      static bool mirrorSeqEnabled();
      // BOT_MIRROR_SNAP_RELOC (read once): read/write the dirty-skip snapshot from the flat
      ...
      // BOT_MIRROR_PREFETCH (read once): software-prefetch the memcmp's cold operand (this unit's
      ...
      // dev Xeon); inert (kill-switch) at 0. Hint tier via BOT_MIRROR_PREFETCH_HINT (t0|t1|t2|nta).
